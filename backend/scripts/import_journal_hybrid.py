#!/usr/bin/env python3
"""
Hybrid Journal Import - Use AI for structuring, manual for tasks
Use AI to structure the journal but extract tasks manually to avoid timeout issues
"""
...
async def import_journal_file(filename: str):
    """Import a single journal file with hybrid approach"""
    
    print(f"\n🚀 HYBRID IMPORT: {filename}")
    print("=" * 60)
    ...
    async for db in get_db():
        try:
            ...
            session = await session_repo.create_session(
                ...
                metadata={"import": True, "file": filename, "hybrid_mode": True}
            )
            ...
            log_step(f"📋 Found {len(extracted_tasks)} tasks to create after AI processing")
...
Please ONLY structure this journal entry into sections and save it. Do NOT create any tasks - I will handle task creation separately. Focus only on organizing the content."""
            ...
            if hasattr(result, 'output'):
                ...
                response_content = "Journal structured and saved successfully"
            
            assistant_message = await message_repo.create_message(
                db, session_id=session.id, role="assistant", content=response_content
            )
...
async def main():
    ...
    if len(sys.argv) != 2:
        print("❌ Usage: uv run python scripts/import_journal_hybrid.py <filename>")
        print("📝 Example: uv run python scripts/import_journal_hybrid.py 20250422T000000.txt")
        print()
